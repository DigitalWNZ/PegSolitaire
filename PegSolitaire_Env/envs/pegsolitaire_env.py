...
class PegSolitaireEnv(Env):
    metadata = {'render_modes': ['human']}
    # reward_range = (-100, +100)

    def __init__(self, render_mode: str | None = None, reward_range=(-100, 100)):
        self._board = Board()
    ...
    def step(self, action_index):
        ...
        terminiated = self._board.terminiated
        if terminiated:
            return obs, 0, terminiated,True, EMPTY_INFO
        if not self._board.is_valid_action(action_index):
            return obs, -100, terminiated,False, EMPTY_INFO
        self._board = self._board.make_move(action_index)
        obs = board_to_obs(self._board)
        terminiated = self._board.terminiated
        reward = self._calculate_final_reward() if terminiated else 0    
        return obs, reward, terminiated, False, EMPTY_INFO
